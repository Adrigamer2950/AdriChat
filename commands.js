...
const admin_db = new db.crearDB('admins');
const mod_db = new db.crearDB('mods');
const bt_db = new db.crearDB('betatesters');
...
module.exports = {
  ...
  checkPermsUser: async function(message, permLvl){
  let obtener = null;
  if(mod_db.tiene(`${message.author.id}`)){
    obtener = await mod_db.obtener(`${message.author.id}`)
    if(1 >= permLvl){
      return true;
    }else{
      return false;
  ...
  if(bt_db.tiene(`${message.author.id}`)){
    obtener = await bt_db.obtener(`${message.author.id}`)
    if(2 >= permLvl){
      return true;
    }else{
      return false;
  ...
  if(admin_db.tiene(`${message.author.id}`)){
    obtener = await admin_db.obtener(`${message.author.id}`)
    if(3 >= permLvl){
      return true;
    }else{
      return false;
  ...
  let userPermLvl = 0;
  if(userPermLvl >= permLvl) {
    return true;
  }
},
  checkPerms: async function(msg, permLvl, args){
    ...
    let result = await this.checkPermsUser(msg, command.permLvl);
    if(result){
      return true;
    }
  ...
  checkValidCmd: async function(msg, args, prefix){
    ...
    if(msg.content.startsWith(prefix) && command != null){
      let result = await this.checkPerms(msg, command.permLvl, args)
      if(result){
        return true;
      }
    }
  ...
  executeCmd: async function(msg, args, client){
    let cmd = this.getCmd(args[0])
    
    if(cmd.checkArgs(msg, args.slice(1))) {
     await cmd.execute(msg, args.slice(1), client)
    }
  }
