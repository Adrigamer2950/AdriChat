module.exports = async (Discord, client, message, prefix, canales_db, obtener, blacklist_db, owner_db, admin_db, interchat_blacklist, canalesp_db, obtenerp) => {
  ...
  if(comprobacion_admins){
   ...
   if(message.channel.id === obtener){
     ...
     channel.createInvite({ unique: true }).then(invite => {
       ...
       .setColor("RANDOM")
       .setDescription(`<:diamond_pickaxe:715531681985069167> **Admin** <:diamond_pickaxe:715531681985069167> | [Invita al bot](https://discord.com/api/oauth2/authorize?client_id=755490775743201330&permissions=388161&scope=bot)`)
   ...
   if(message.channel.id === obtenerp){
     ...
     channel.createInvite({ unique: true }).then(invite => {
       ...
       .setAuthor(`${message.author.tag} | {message.author.id}`, message.author.displayAvatarURL({ dynamic: true }))
       .setColor("#188cb8")
       .setDescription(`<:diamond_pickaxe:715531681985069167> **Admin** <:diamond_pickaxe:715531681985069167> | [Invita al bot](https://discord.com/api/oauth2/authorize?client_id=755490775743201330&permissions=388161&scope=bot) | Canal privado`)
       .addField(`<a:peepo_clap:722055764826652824> Mensaje:`, message.content)
       .setThumbnail(message.guild.iconURL())
       .setFooter(`${message.guild.name}, ${message.guild.memberCount} miembros`, message.guild.iconURL({ dynamic: true }));
