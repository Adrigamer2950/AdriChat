module.exports = async (Discord, client, message, prefix, canales_db, obtener, blacklist_db, owner_db, admin_db, mod_db, bt_db, interchat_blacklist, canalesp_db, obtenerp) => {
  if (message.author.bot) return;
  let comprobacion_admins = await admin_db.obtener(`${message.author.id}`);
  if(comprobacion_admins) return;
  let comprobacion_mods = await mod_db.obtener(`${message.author.id}`);
  if(comprobacion_mods) return;
  let comprobacion_owners = await owner_db.obtener(`${message.author.id}`);
  if(comprobacion_owners) return;
  let comprobacion_bts = await bt_db.obtener(`${message.author.id}`);
  if(comprobacion_bts) return;
  ...
  if(comprobacion_blacklist){
   ...
   if(message.channel.id === obtener){
     ...
     channel.createInvite({ unique: true }).then(invite => {
       ...
       .setColor("RANDOM")
       .setDescription(`:busts_in_silhouette: **User** :busts_in_silhouette: | [Invita al bot](https://discord.com/api/oauth2/authorize?client_id=755490775743201330&permissions=388161&scope=bot)`)
   ...
   if(message.channel.id === obtenerp){
     ...
     channel.createInvite({ unique: true }).then(invite => {
       ...
       .setColor("#188cb8")
       .setDescription(`:busts_in_silhouette: **User** :busts_in_silhouette: | [Invita al bot](https://discord.com/api/oauth2/authorize?client_id=755490775743201330&permissions=388161&scope=bot) | Canal privado`)
       .addField(`<a:peepo_clap:722055764826652824> Mensaje:`, message.content)
       .setThumbnail(message.guild.iconURL())
       .setFooter(`${message.guild.name}, ${message.guild.memberCount} miembros`, message.guild.iconURL({ dynamic: true }));
